...
from app.services.security_service import get_access_tokens_data, get_refresh_tokens_data
...
@router.post("/auth")
async def auth(user_in: UserLoginDto):
    logging.info('POST: /login.')
    return await security_login(user_in)


@router.get("/refresh")
async def refresh(data: tuple[Token, UUID] = Depends(get_refresh_tokens_data)):
