...
@router.get("/users")
async def users(current_user: str = Depends(get_access_tokens_data)):
    logging.info('GET: /users.')
    return await get_users()


@router.get("/user")
async def user(id: UUID, current_user: str = Depends(get_access_tokens_data)):
    logging.info('GET: /user.')
    return await get_user(id)


@router.post("/create_user")
async def create(new_user: UserCreateDto, current_user: str = Depends(get_access_tokens_data)):
    logging.info('POST: /create_user.')
    return await create_user(new_user, current_user)


@router.patch("/update_user")
async def update(updated_user: UserUpdateDto, current_user: str = Depends(get_access_tokens_data)):
    logging.info('PATCH: /update_user.')
    return await update_user(updated_user, current_user)


@router.delete("/delete_user")
async def delete(deleted_user: UserDeleteDto, current_user: str = Depends(get_access_tokens_data)):
